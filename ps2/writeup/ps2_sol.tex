...
2 \\
The recursion stack goes down n levels and stops at n = 0.
...
4 \\
First, the function splits the edge into 3 equal parts, then produces $4^i$ nodes for each part.
...
2 \\
For a single node, you only need to draw one triangle for each side, so the runtime is constant.
...
4 \\
If there are $3 * 4^i$ nodes, and drawing each node individually takes constant time, then we can just multiply both of them and remove the constant.
...
1 \\
Line segments are rendered at the last level, $n$, so for some $i$ below it, the cost is 0.
...
1 \\
Line segments are rendered at the last level.
%%% PROBLEM 1(j) ANSWER END %%%
}
\newcommand{\answerIk}{ \answer
%%% PROBLEM 1(k) ANSWER START %%%
4 \\
We render $3*4^i$ nodes in constant time.
%%% PROBLEM 1(k) ANSWER END %%%
}
\newcommand{\answerIl}{ \answer
%%% PROBLEM 1(l) ANSWER START %%%
4 \\
The total time is the sum of of the cost of rendering the last level and at each level, so it's just $\Theta(4^n)$.
...
4 \\
At level 0, the length of each side is 1, and with each subsequent level up to $n-1$, the sides get split into 4 equal parts of length $l/3$. So for each level $i$, the length of each side is $1/(3^i)$.
...
3 \\
We render $3 * 4^i$ nodes in $\Theta(1/(3^n))$ time, so just multiply those together.
...
There are $n$ levels in the tree and a total of $3 * 4^i$ nodes on each level $i$. In 3D rendering, each node on level $i < n$ renders one triangle and there is no rendering on level $n$. In 2D rendering, the length of a triangle's side at level $i$ is $1/(3^{i+1})$ for $i < n$, so the rendering time for each node is $1/(9^{i+1})$. \\\\
Ultimately, the total rendering time is the number of nodes times the rendering time of each node, or $(3*4^i) * 1/(9^{i+1}) = 3 * (4^i/9^{i+1}) = \Theta(1)$.
...
3 \\
It searches the entire queue for the min.
